...
class Sign extends Component {
  ...
  handleDelete(id) {
    return () => {
      ...
      confirm({
        ...
        onOk() {
          appActions.loading(true).then(() => {
            return contentManageActions.deleteSign({
              id
            });
          }).then(() => {
            return contentManageActions.getSignList({
              pageNum: 1,
              pageSize
            })
          }).then(() => {
  ...
  handleChangeState(id, status) {
    return () => {
      ...
      confirm({
        ...
        onOk() {
          appActions.loading(true).then(() => {
            return contentManageActions.changeSignState({
              id,
              flag: changedStatus,
              weight: status === 0 ? signSort.get('0') + 1 : null
            });
          }).then(() => {
            return Promise.all([
              contentManageActions.getSignList({
  ...
  render() {
    ...
    const signList = contentManage.get('signList') && contentManage.get('signList');
    const list = signList && signList.get('list') && signList.get('list');
    const total = signList && signList.get('totalCount') && signList.get('totalCount');
    const current = signList && signList.get('currentPage') && signList.get('currentPage');
    const editData = signList && list.find(v => v.id === this.state.editId);
    const { pageSize } = this.state
    const columns = [{
      title: '序号',
      key: 'id',
      ...
      render: (action, data, index) => {
        return (<span>
          ...
          <a style={{marginLeft: '8px'}} onClick={this.goToSubEntry(data.id)}>子标签</a>
        </span>)
     }
    }];
    const pagination = {
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(Sign))
